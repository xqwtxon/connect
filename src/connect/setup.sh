...
curl -sL "https://raw.githubusercontent.com/xqwtxon/connect.sh/stable/src/connect/connect" -o "$TEMP/connect.exec"
if [ ! "$?" == "0" ]; then
	echo "error" && echo "[!] Please check your connection and try again!" && exit 1
else echo "done"
fi
echo -n "[*] Downloading Update File: "
curl -sL "https://raw.githubusercontent.com/xqwtxon/connect.sh/stable/src/connect/update.sh" -o "$TEMP/connect.update.sh"
if [ ! "$?" == "0" ]; then
	echo "error" && echo "[!] Please check your connection and try again!" && exit 1
else echo "done"
fi
